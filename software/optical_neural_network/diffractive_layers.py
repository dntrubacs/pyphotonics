...
class InputLayer(torch.nn.Module):
    ...
    def __init__(self, size: int, length: float,
        ...
        self.weights = torch.ones(size=(size, size))
    ...
    def forward(self, x) -> torch.tensor:
        ...
        """
        self.weights = torch.mul(x, self.weights)
        ...
        optical_modes = find_optical_modes(
            source_matrix_position=self.neuron_coordinates,
            detector_matrix_position=self.neuron_coordinates_next,
            source_matrix_optical_mode=self.weights,
            wavelength=self.wavelength
        )
...
class DetectorLayer(torch.nn.Module):
    ...
    Attributes:
        size: The numbers of pixels in a given collumn or row (the total
...
if __name__ == '__main__':
    ...
    debug_input_layer = InputLayer(
        size=10, length=1, z_coordinate=0.0, z_next=1.0, size_next=10,
        wavelength=652E-9
    )
    debug_diffractive_layer = DiffractiveLayer(
        size=10, length=1, z_coordinate=1.0, z_next=2.0, wavelength=652E-9)
    debug_detector_layer = DetectorLayer(size=10, length=1.0, z_coordinate=2.0)
    output = debug_input_layer(input)
